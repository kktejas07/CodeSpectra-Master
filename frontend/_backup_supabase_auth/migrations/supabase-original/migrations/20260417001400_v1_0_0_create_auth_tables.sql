...
-- Defaults use gen_random_uuid() (built-in); uuid_generate_v4() may be missing from search_path on hosted Supabase.
...
CREATE INDEX IF NOT EXISTS idx_users_face_enrollment_status ON users(face_enrollment_status);
...
CREATE INDEX IF NOT EXISTS idx_face_enrollments_angle_type ON face_enrollments(angle_type);
CREATE INDEX IF NOT EXISTS idx_face_enrollments_status ON face_enrollments(status) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_face_enrollments_user_status ON face_enrollments(user_id, status);
...
CREATE INDEX IF NOT EXISTS idx_face_login_attempts_user_id ON face_login_attempts(user_id);
CREATE INDEX IF NOT EXISTS idx_face_login_attempts_status ON face_login_attempts(status);
CREATE INDEX IF NOT EXISTS idx_face_login_attempts_created_at ON face_login_attempts(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_face_login_attempts_user_created ON face_login_attempts(user_id, created_at DESC);
...
-- 20260417001200 may have already created public.oauth_tokens with fewer columns; extend in place
-- so indexes below succeed. One ADD per statement avoids parser/transaction edge cases.
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS provider_user_id VARCHAR(255);
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS provider_email VARCHAR(255);
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS provider_avatar_url TEXT;
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS token_type VARCHAR(50) DEFAULT 'Bearer';
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS expires_in INT;
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ;
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS last_used_at TIMESTAMPTZ;
ALTER TABLE public.oauth_tokens ADD COLUMN IF NOT EXISTS version INT DEFAULT 1;

DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'oauth_tokens' AND column_name = 'token_expires_at'
  ) THEN
    UPDATE public.oauth_tokens
    SET expires_at = COALESCE(expires_at, token_expires_at::timestamptz)
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'oauth_tokens' AND column_name = 'last_used'
  ) THEN
    UPDATE public.oauth_tokens
    SET last_used_at = COALESCE(last_used_at, last_used::timestamptz)
    WHERE last_used_at IS NULL AND last_used IS NOT NULL;
  END IF;
END $$;

UPDATE public.oauth_tokens SET provider_user_id = '' WHERE provider_user_id IS NULL;

-- Indexes for oauth_tokens (qualified: search_path may omit public during migrations)
...
CREATE INDEX IF NOT EXISTS idx_oauth_tokens_provider_user_id ON public.oauth_tokens(provider, provider_user_id);
...
CREATE INDEX IF NOT EXISTS idx_sessions_access_token ON sessions(access_token) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_sessions_refresh_token ON sessions(refresh_token) WHERE is_active = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_data_versions_record_id ON data_versions(record_id);
CREATE INDEX IF NOT EXISTS idx_data_versions_table_name ON data_versions(table_name);
CREATE INDEX IF NOT EXISTS idx_data_versions_user_id ON data_versions(user_id) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_data_versions_migration_id ON data_versions(migration_id);
CREATE INDEX IF NOT EXISTS idx_data_versions_created_at ON data_versions(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_data_versions_action ON data_versions(action);
...
CREATE TABLE IF NOT EXISTS migrations_log (
  ...
  -- Row revision (avoid duplicate name with migration_version)
  row_revision INT DEFAULT 1
);

-- Indexes for migrations_log
CREATE INDEX IF NOT EXISTS idx_migrations_log_status ON migrations_log(status);
CREATE INDEX IF NOT EXISTS idx_migrations_log_migration_version ON migrations_log(migration_version DESC);
CREATE INDEX IF NOT EXISTS idx_migrations_log_created_at ON migrations_log(created_at DESC);
...
CREATE TABLE IF NOT EXISTS release_versions (
  ...
  -- Row revision (avoid duplicate name with release_tag)
  row_revision INT DEFAULT 1
);

-- Indexes for release_versions
CREATE INDEX IF NOT EXISTS idx_release_versions_release_tag ON release_versions(release_tag DESC);
CREATE INDEX IF NOT EXISTS idx_release_versions_status ON release_versions(status);
CREATE INDEX IF NOT EXISTS idx_release_versions_deployed_at ON release_versions(deployed_at DESC);
...
-- Optional app-level migration log skipped: remote may already have public.migrations_log
-- with a different shape; Supabase tracks applied files in supabase_migrations.schema_migrations.
