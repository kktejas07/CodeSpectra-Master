...
async function executeMigrations() {
  console.log('[CodeSpectra] Starting Supabase migrations...');
  ...
  for (const file of migrationFiles) {
    try {
      ...
      console.log(`[CodeSpectra] Executing ${file}...`);
      ...
      for (const statement of statements) {
        if (statement.trim()) {
          ...
          if (error) {
            console.warn(`[CodeSpectra] Warning in ${file}: ${error.message}`);
          }
        }
      }
      
      console.log(`[CodeSpectra] ✓ ${file} completed`);
    } catch (error) {
      console.error(`[CodeSpectra] Error executing ${file}:`, error.message);
    }
  }
}

async function seedData() {
  console.log('[CodeSpectra] Seeding database with initial data...');
  ...
  try {
    ...
    if (plansError) console.warn('[CodeSpectra] Plans warning:', plansError.message);
    else console.log('[CodeSpectra] ✓ Subscription plans seeded');
    ...
    if (challengesError) console.warn('[CodeSpectra] Challenges warning:', challengesError.message);
    else console.log('[CodeSpectra] ✓ Sample challenges seeded');

  } catch (error) {
    console.error('[CodeSpectra] Error seeding data:', error.message);
  }
}

async function main() {
  try {
    console.log('[CodeSpectra] Connected to Supabase');
    await executeMigrations();
    await seedData();
    console.log('[CodeSpectra] ✓ All migrations and seeding completed successfully!');
  } catch (error) {
    console.error('[CodeSpectra] Migration failed:', error);
    process.exit(1);
  }
}
