...
export default function PricingAdminPage() {
  ...
  const handleCreateTier = async (data) => {
    try {
      ...
      console.error('[CodeSpectra] Error creating tier:', error);
    }
  };
  ...
  const handleToggleFeature = async (tierId, featureId, isEnabled) => {
    try {
      ...
      console.error('[CodeSpectra] Error toggling feature:', error);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Configure catalog tiers and feature flags. Stripe API keys and <code className="rounded bg-muted px-1 text-xs">price_…</code> IDs for
            Checkout live under Platform settings → Stripe & billing.
          </p>
        </div>
