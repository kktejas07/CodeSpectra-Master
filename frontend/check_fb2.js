const admin = require('firebase-admin');
admin.initializeApp({
  credential: admin.credential.cert({
    projectId: 'codespectra-f60cf',
    clientEmail: 'firebase-adminsdk-fbsvc@codespectra-f60cf.iam.gserviceaccount.com',
    privateKey: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
  }),
});
const auth = admin.auth();

async function check() {
  const projectConfig = await auth.projectConfigManager().getProjectConfig();
  console.log('Authorized domains:', projectConfig.authorizedDomains);
  
  try {
    const googleConfig = await auth.getProviderConfig('google.com');
    console.log('Google provider:', JSON.stringify(googleConfig, null, 2));
  } catch(e) { console.log('Google:', e.message); }
  
  try {
    const githubConfig = await auth.getProviderConfig('github.com');
    console.log('GitHub provider:', JSON.stringify(githubConfig, null, 2));
  } catch(e) { console.log('GitHub:', e.message); }
  
  try {
    const pwConfig = await auth.getProviderConfig('password');
    console.log('Password provider:', JSON.stringify(pwConfig, null, 2));
  } catch(e) { console.log('Password:', e.message); }
}
check().catch(console.error).finally(() => process.exit());
