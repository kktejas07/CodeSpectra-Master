...
export async function initializeSampleData(projectId: string = 'demo-project') {
  try {
    console.log('[CodeSpectra] Initializing sample SonarQube data...')
    ...
    if (gateError) console.error('[CodeSpectra] Error creating gate:', gateError)
    else console.log('[CodeSpectra] Quality gate created:', gateData?.[0])
    ...
    if (ratingError) console.error('[CodeSpectra] Error creating rating:', ratingError)
    else console.log('[CodeSpectra] Quality rating created:', ratingData?.[0])
    ...
    if (metricsError) console.error('[CodeSpectra] Error creating metrics:', metricsError)
    else console.log('[CodeSpectra] File metrics created:', metricsData?.length)
    ...
    if (issuesError) console.error('[CodeSpectra] Error creating issues:', issuesError)
    else console.log('[CodeSpectra] Issues created:', issuesData?.length)
    ...
    if (activitiesError) console.error('[CodeSpectra] Error creating activities:', activitiesError)
    else console.log('[CodeSpectra] Activities created:', activitiesData?.length)

    console.log('[CodeSpectra] Sample data initialization completed!')
    return { success: true }
  } catch (error) {
    console.error('[CodeSpectra] Error initializing sample data:', error)
...
export async function clearProjectData(projectId: string) {
  try {
    console.log('[CodeSpectra] Clearing data for project:', projectId)
    ...
    console.log('[CodeSpectra] Project data cleared')
    return { success: true }
  } catch (error) {
    console.error('[CodeSpectra] Error clearing project data:', error)
    return { success: false, error }
  }
}
