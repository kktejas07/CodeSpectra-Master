...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[CodeSpectra] Error fetching ratings:', error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[CodeSpectra] Error creating rating:', error)
...
**Task 5.3: Deploy**
```bash
# Build, then deploy per your host (Docker, Supabase Edge + static, VPS, managed Node, etc.)
npm run build

# Or push to git for CI/CD auto-deploy
git add .
