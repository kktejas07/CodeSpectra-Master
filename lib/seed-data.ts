...
export function isSeedDataAllowed(): boolean {
  if (isProduction()) {
    console.error('[v0] Seed data operations are disabled in production')
    return false
  }
  return true
}
...
export async function seedDemoUsers() {
  ...
  try {
    console.log('[v0] Seeding demo user accounts...')
...
export async function seedSampleProjects() {
  ...
  try {
    console.log('[v0] Seeding sample projects...')
    ...
    console.error('[v0] Error seeding sample projects:', error)
  }
}
...
export async function cleanupSeedData() {
  if (isProduction()) {
    console.error('[v0] Refusing to cleanup seed data in production')
    return { success: false, message: 'Cannot cleanup seed data in production' }
  }

  try {
    console.log('[v0] Cleaning up seed data...')
    ...
    console.log('[v0] Seed data cleanup complete')
    return { success: true, message: 'Seed data cleaned up successfully' }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    console.error('[v0] Error cleaning up seed data:', error)
...
export async function initializeSeedData() {
  if (!isSeedDataAllowed()) {
    console.log('[v0] Seed data initialization skipped (production environment)')
    return
  }

  console.log(
    `[v0] Initializing seed data for ${getEnvironmentType()} environment...`
  )

  try {
    await seedDemoUsers()
    await seedSampleProjects()
    console.log('[v0] Seed data initialization complete')
  } catch (error) {
    console.error('[v0] Error initializing seed data:', error)
  }
}
  ...
  try {
    ...
    for (const table of tables) {
      ...
      if (!error && data) {
        ...
        if (count > 0) {
          console.warn(`[v0] Found ${count} seed records in ${table}`)
        }
      }
    }
    ...
    console.error('[v0] Error verifying seed data cleanup:', error)
