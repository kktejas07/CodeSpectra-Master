...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[v0] API Error in GET /api/scanner/metrics/files:', error)
    return NextResponse.json(
...
export async function getFileMetricsDetail(fileId: string) {
  try {
    ...
    console.error('[v0] Error fetching file metrics detail:', error)
    throw error
  }
}
...
src/components/scanner/quality-gate-dashboard.tsx,typescript,412,12,6,72.3,4.5`
    ...
    console.error('[v0] API Error in POST /api/scanner/metrics/files:', error)
    return NextResponse.json(
